...
def cluster_end(chars, start, sel_cps):
    ...
    """
    index = start + 1
    if ord(chars[start]) in REGIONAL and index < len(chars) \
...
def do_inspect(text, selectors, pua2base):
    ...
    counts = dict.fromkeys(
        ("assumed_human", "explicit_human", "ai_vs", "ai_pua", "unknown",
         "edited", "mixed", "whitespace"), 0)
    ...
    while index < len(chars):
        ...
        if index < len(chars) and ord(chars[index]) in sel2name:
            ...
            index += 1
            counts["explicit_human" if name == "human" else
                   "ai_vs" if name == "ai" else name] += 1
        else:
            counts["assumed_human"] += 1

    lines = ["characters: {}".format(len(chars))]
    for key in ("assumed_human", "explicit_human", "ai_vs", "ai_pua",
                "unknown", "edited", "mixed", "whitespace"):
        lines.append("{}: {}".format(key, counts[key]))
    lines.append("unrecognised_selectors: {}".format(
        " ".join("U+{:04X}".format(cp) for cp in sorted(unrecognised_selectors)) or "-"))
    lines.append("unrecognised_pua: {}".format(
        " ".join("U+{:04X}".format(cp) for cp in sorted(unrecognised_pua)) or "-"))
    return "\n".join(lines) + "\n"
...
def do_mark(text, state, mode, selectors, pua2base, base2pua):
    ...
    while index < len(chars):
        ...
        if index < len(chars) and ord(chars[index]) in sel_cps:
            ...
            continue
        if mode == "pua" and state == "ai" and len(cluster) == 1 and cp in base2pua:
...
def do_convert(text, from_mode, to_mode, selectors, pua2base, base2pua):
    ...
    if from_mode == "vs":
        ...
        while index < len(chars):
            char = chars[index]
            index += 1
            if (index < len(chars) and ord(chars[index]) == vs_ai
...
def selftest():
    ...
    check(mark(direct, mode="pua") == direct, "mark --mode=pua is not idempotent")
    ...
    check(convert(marked_human, "vs", "pua") == marked_human,
    ...
    for text in (flag, family, tone, emoji_vs):
        check(mark(text, mode="pua") == text + ai,
    ...
    check("edited: 1" in do_inspect(reserved, selectors, pua2base),
...
def build_parser():
    ...
    def add_common(sub):
        sub.add_argument("file", metavar="FILE", help="input file, or - for stdin")
        sub.add_argument("-o", "--output", help="write to this file instead of stdout")
    ...
    for state in GENERATED_STATES:
        group.add_argument("--" + state, dest="state", action="store_const",
                           const=state)
    marker.add_argument("--mode", choices=("vs", "pua"), default="vs")
    add_common(marker)

    converter = subs.add_parser("convert", help="convert between encodings")
    converter.add_argument("--from", dest="from_mode", choices=("vs", "pua"),
                           required=True)
    converter.add_argument("--to", dest="to_mode", choices=("vs", "pua"),
                           required=True)
...
def main(argv=None):
    ...
    if args.command == "mark" and args.mode == "pua" and args.state != "ai":
        parser.error("--mode=pua is only valid with --ai "
                     "(there is no PUA encoding for human or unknown)")
    try:
        _, selectors, pua2base, base2pua = load_mapping(args.mapping)
    except (OSError, ValueError, KeyError) as error:
        sys.stderr.write("{}: cannot load mapping: {}\n".format(PROG, error))
        return 1
    try:
        text = read_input(args.file)
    except OSError as error:
        sys.stderr.write("{}: cannot read input: {}\n".format(PROG, error))
        return 1

    if args.command == "inspect":
        result = do_inspect(text, selectors, pua2base)
    elif args.command == "mark":
        result = do_mark(text, args.state, args.mode, selectors, pua2base,
                          base2pua)
    elif args.command == "convert":
        result = do_convert(text, args.from_mode, args.to_mode, selectors,
                            pua2base, base2pua)
    else:
        sys.stderr.write("{}: strip is lossy: provenance cannot be recovered "
                         "from the output\n".format(PROG))
