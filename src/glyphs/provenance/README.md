...
Three states are generated: human, AI, and unknown. Two further states
(`mixed` and `edited`) are proposed in the registry but not generated by the
patcher (see below). Unmarked text has no in-band state; the protocol takes no
position on what it means.
...
| --------------- | ------------------ | ----------------------- |
| Human           | `U+E0100` (`VS17`) | Generated               |
| AI              | `U+E0101` (`VS18`) | Generated               |
| Mixed or other  | `U+E0102` (`VS19`) | Proposed; not generated |
| Human-edited AI | `U+E0103` (`VS20`) | Proposed; not generated |
...
- **Cost.** About +1 s on a roughly 37 s Hack `--complete` build, and about
  4.5 % more glyphs.
- **Proposed selectors.** `U+E0102` (`mixed`) and `U+E0103` (`edited`) have no
  variants in the font and fall through to the base glyph.
