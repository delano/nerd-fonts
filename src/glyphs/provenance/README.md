...
The patcher currently generates variants for human, AI, and unknown. The
protocol's Generated set is now human, AI, and mixed (see the states table
in [SPEC.md](https://github.com/textprov/textprov/blob/main/SPEC.md#states)):
the font-patcher lags one state behind the spec and still emits `.unknown`
where it should emit `.mixed`. The other two states (`edited`, and now
`unknown`) are Proposed in the registry and generated by no producer.
Unmarked text has no in-band state; the protocol takes no position on what
it means.
...
| State           | Selector           | Protocol status         | Font-patcher today |
| --------------- | ------------------ | ----------------------- | ------------------ |
| Human           | `U+E0100` (`VS17`) | Generated               | Emits `.human`     |
| AI              | `U+E0101` (`VS18`) | Generated               | Emits `.ai`        |
| Mixed or other  | `U+E0102` (`VS19`) | Generated               | Not yet emitted    |
| Human-edited AI | `U+E0103` (`VS20`) | Proposed; not generated | Not yet emitted    |
| Unknown         | `U+E0104` (`VS21`) | Proposed; not generated | Emits `.unknown` (spec drift; to be dropped) |
...
`.human` is never marked, in any style. A visible mark on human text would
make explicitly-marked human differ visually from unmarked text, which M1
forbids.
...
- **Cost.** About +1 s on a roughly 37 s Hack `--complete` build, and about
  4.5 % more glyphs.
- **Unemitted selectors.** `U+E0102` (`mixed`) and `U+E0103` (`edited`) have
  no variants in the font and fall through to the base glyph. Under contract
  version 1 `mixed` is Generated; the patcher will catch up.
